...
def region_selection(image):
    ...
    Parameters:
        image: we pass here the output from canny where we have identified edges in the frame
    """
...
def pixel_points(y1, y2, line):
    ...
    if abs(slope) > 1e-6:  # Check if slope is not too close to zero
...
def process_video(test_video, output_video):
    ...
    while(cap.isOpened()):
        ...
        # Convert the processed frame to BGR format before writing to the video
...
def process_image(image_path):
    """
    Process a single image to detect lane lines.
    Parameters:
...
process_image('C:\\Users\\hegde\\OneDrive\\Desktop\\LaneDet\\img5.jpg')
